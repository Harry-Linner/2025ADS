# 🥝 Maximum Safe Fruit Set Project Report

## 1. Background Introduction

### 1.1 Project Description and Problem Definition

- Problem Context: Many dietary rules stipulate that certain fruits cannot be consumed together, as this can lead to adverse health effects (e.g., the conflict between bananas and cantaloupes) .
- Project Goal: Select the largest quantity of mutually non-conflicting fruits from a basket. If multiple sets have the maximum quantity, the secondary goal is to select the set with the minimum total price .
- Input and Output: The input includes the number of conflicts $N$, the number of fruits $M$, a list of conflict pairs, and a list of fruit prices. The required output includes the maximum quantity, the list of fruit IDs, and the total price .

### 1.2 Problem Model and Algorithm Overview

- Problem Modeling: Each fruit is abstracted as a vertex in a graph, and every conflicting pair of fruits is abstracted as an edge .
- Model Transformation: Solving the "Maximum Safe Fruit Set" problem is transformed into solving the Maximum Independent Set (MIS) problem on the graph, with the additional optimization constraint of minimizing the total price.
- Algorithm Selection: Given that the Maximum Independent Set problem is NP-Hard, an efficient Backtracking Search (Depth First Search, DFS) combined with pruning strategies is employed for the solution .

### 1.3 Computational Challenges and Solutions

- Challenge: For cases where the number of fruits $M$ is large (e.g., $M > 80$), the algorithm's runtime grows exponentially, making it difficult to find a solution within a short time .
- Solution Approach: More useful pruning and preprocessing techniques must be introduced to the basic backtracking search to significantly improve search efficiency.
...
- A.2 Random Input Generation Tool: `project3_randominput.c`
